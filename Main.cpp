...
int main() {
	...
	test_operator_pls1_comp(a, b);
	c = d;
	test_operator_pls1_comp(a, b);
	test_operator_pls1_comp(f, b);
	test_operator_pls2_comp(g, "asdf");
	test_operator_pls2_comp(d, " ");
	test_operator_pls2_comp(k, "asdf");
	...
	for (int i = 0; i < 5; i++) {
		...
		if (test_sum(pt[i + 1], "def")) n++;
	}

	cout << "test sum is OK: " << n << "/5" << endl << endl;
	n = 0;

	for (int i = 0; i < 5; i++) {

	if (test_search(pt[i + 1], "a")) n++;
	}

	cout << "test search is OK: " << n << "/5" << endl << endl;
}
